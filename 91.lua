...
local antiStiffConnection = nil
local antiStiffHeartbeat = nil
local haloPart = nil
local haloWeld = nil
...
-- ========== 静默甩飞功能 ==========
local function enableAntiStiff()
    Settings.AntiStiff = true
    safeCall(function()
        if antiStiffConnection then antiStiffConnection:Disconnect() end
        if antiStiffHeartbeat then antiStiffHeartbeat:Disconnect() end
        
        antiStiffConnection = RunService.Stepped:Connect(function()
            if not Settings.AntiStiff then return end
        ...
        antiStiffHeartbeat = RunService.Heartbeat:Connect(function()
            if not Settings.AntiStiff then return end
            local char = Player.Character
...
local function disableAntiStiff()
    Settings.AntiStiff = false
    safeCall(function()
        if antiStiffConnection then
            antiStiffConnection:Disconnect()
            antiStiffConnection = nil
        end
        if antiStiffHeartbeat then
            antiStiffHeartbeat:Disconnect()
            antiStiffHeartbeat = nil
        end
    end)
end
...
Player.CharacterAdded:Connect(function(char)
    ...
    if Settings.AntiStiff then safeCall(enableAntiStiff) end
end)
    ...
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= Player then
            ...
            if Settings.EspInfo then
                ...
                healthText.Text = string.format("❤️ %d/%d", health, maxHealth)
                ...
                distText.Text = string.format("📏 %.0fm", distance)
        ...
        game:GetService("Players").LocalPlayer.Chatted:Connect(function(msg)
            if msg:lower() == "/antikick" then
                ...
                if MasterEnabled then
                    toggleAntiKick(true)
                    WindUI:Notify({Title = "🛡️ 防踢已开启", Content = "已激活全部防御", Duration = 3})
                else
                    WindUI:Notify({Title = "🛡️ 防踢已关闭", Content = "已关闭防御", Duration = 3})
                end
            end
        end)
    end)
...
local function loadFlyScript()
    safeCall(function()
        ...
        up.Text = "上升"
        up.TextColor3 = Color3.fromRGB(255, 215, 0)
        up.TextSize = 14
        ...
        down.Text = "下降"
        down.TextColor3 = Color3.fromRGB(255, 215, 0)
        down.TextSize = 14
        ...
        onof.Text = "飞行"
        onof.TextColor3 = Color3.fromRGB(255, 215, 0)
        onof.TextSize = 14
        ...
        mine.Text = "速度-1"
        mine.TextColor3 = Color3.fromRGB(255, 215, 0)
        ...
        mini.Parent = Frame
        mini.BackgroundColor3 = Color3.fromRGB(255, 165, 0)
        mini.BackgroundTransparency = 0.5
        mini.Font = "SourceSans"
        mini.Size = UDim2.new(0, 45, 0, 28)
        mini.Text = "隐藏"
        mini.TextColor3 = Color3.fromRGB(255, 215, 0)
        mini.TextSize = 30
        mini.Position = UDim2.new(0, 44, -1, 27)
        
        local mini2 = Instance.new("TextButton")
        mini2.Parent = Frame
        mini2.BackgroundColor3 = Color3.fromRGB(255, 165, 0)
        mini2.BackgroundTransparency = 0.5
        mini2.Font = "SourceSans"
        mini2.Size = UDim2.new(0, 45, 0, 28)
        mini2.Text = "+"
        mini2.TextColor3 = Color3.fromRGB(255, 215, 0)
        mini2.TextSize = 40
        ...
        WindUI:Notify({Title = "✈️ 飞行面板已打开", Content = "点击飞行按钮开始飞行", Duration = 3})
    end)
end
...
local function createMainGUI()
    ...
    Tabs.General:Toggle({
        Title = "静默甩飞",
        Value = Settings.AntiStiff,
        Callback = function(v)
            if v then enableAntiStiff() else disableAntiStiff() end
        end
    })
...
local function showVerificationUI()
    ...
    inputBox.PlaceholderText = "请输入: dawn"
    ...
    buttonContainer.Position = UDim2.new(0.05, 0, 0, 140)
    buttonContainer.BackgroundTransparency = 1
    buttonContainer.Parent = mainFrame
    
    local exitBtn = Instance.new("TextButton")
    exitBtn.Size = UDim2.new(0.45, -5, 1, 0)
    exitBtn.Position = UDim2.new(0, 0, 0, 0)
    exitBtn.BackgroundColor3 = Color3.fromRGB(60, 60, 70)
    ...
    confirmBtn.Size = UDim2.new(0.45, -5, 1, 0)
    confirmBtn.Position = UDim2.new(0.55, 0, 0, 0)
    confirmBtn.BackgroundColor3 = Color3.fromRGB(255, 70, 120)
