    ...
    if char then
        ...
        if head then
            for _, child in ipairs(head:GetChildren()) do
                if child:IsA("BasePart") and child.Name:find("Team") then
                    return child
                end
            end
        end
        ...
        if hum then
            if hum.DisplayName and hum.DisplayName ~= "" then
                local teamName = hum.DisplayName
                if teamName:find("Team") or teamName:find("队伍") then
                    return teamName
                end
            end
        end
...
local function isTeammate(plr)
    ...
    if myTeam and theirTeam then
        if type(myTeam) == "table" and type(theirTeam) == "table" then
            return myTeam == theirTeam
        elseif type(myTeam) == "string" and type(theirTeam) == "string" then
            return myTeam == theirTeam
        elseif type(myTeam) == "Instance" and type(theirTeam) == "Instance" then
            return myTeam == theirTeam
        end
    end
    return false
...
-- ========== 上帝模式（无敌 - 按钮版） ==========
local function createGodModeButton()
    if godModeGui then
        godModeGui:Destroy()
        godModeGui = nil
    end
    ...
    godModeGui.Name = "GodModeGUI"
    godModeGui.Parent = Player:WaitForChild("PlayerGui")
    godModeGui.ResetOnSpawn = false
    
    godModeButton = Instance.new("TextButton")
    godModeButton.Parent = godModeGui
    godModeButton.Size = UDim2.new(0, 150, 0, 50)
    godModeButton.Position = UDim2.new(0.5, -75, 0.5, -25)
    ...
    godModeButton.Text = "无敌: 关闭"
    godModeButton.BorderSizePixel = 2
    godModeButton.BorderColor3 = Color3.fromRGB(255, 50, 50)
    ...
    godModeButton.MouseButton1Click:Connect(function()
        ...
        if isGodModeActive then
            godModeButton.Text = "无敌: 开启"
            godModeButton.BackgroundColor3 = Color3.fromRGB(0, 150, 0)
            godModeButton.BorderColor3 = Color3.fromRGB(0, 255, 0)
        ...
        if plr ~= Player then
            ...
            if Settings.EspInfo then
                ...
                if isTeam then
                    ...
                    teamText.Position = displayPos + Vector2.new(0, 35)
                    teamText.Text = "[队友]"
                    teamText.Color = Color3.fromRGB(0, 255, 0)
                    teamText.Size = 11
                    teamText.Center = true
                    teamText.Outline = true
                    teamText.OutlineColor = Color3.new(0, 0, 0)
                    table.insert(espTexts, teamText)
                end
            end
        end
    end
...
local function addHighlightToPlayer(p)
    ...
    local function add()
        ...
        if isTeammate(p) then
            ...
            e.OutlineColor = Color3.fromRGB(0, 200, 0)
        else
...
local function createMainGUI()
    ...
    -- 上帝模式按钮说明
    local godLabel = Tabs.General:Label("上帝模式按钮已显示在屏幕中央")
    ...
    -- ===== 玩家操作 =====
    Tabs.Combat:Section({ Title = "玩家操作" })
    ...
    local playerDropdown = Tabs.Combat:Dropdown({
        ...
        Callback = function(name)
            if name then
                ...
                selectedKillPlayer = Players:FindFirstChild(name)
            end
        end
    })
